...
instance: "{vm-name}"        # VM identifier (edge-vm, db-vm, etc.)
...
node_cpu_seconds_total{instance="edge-vm"}             # CPU usage by VM
container_memory_usage_bytes{tier="data"}              # Container memory by tier
node_filesystem_avail_bytes{instance="media-vm"}       # Available disk space
...
{instance="db-vm"}                                      # All logs from db VM
{instance="edge-vm",container="traefik"}               # Traefik logs from edge VM
